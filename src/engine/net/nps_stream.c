...
NpsStream *nps_stream_new(int fd, nps_tls_t *tls) {
    NpsStream *s = calloc(1, sizeof(NpsStream));
    if (!s) return NULL;
    s->fd = fd;
    s->tls = tls;
    return s;
...
static void throttle(NpsStream *s, size_t bytes) {
    if (!s || s->limit_rate == 0 || bytes == 0) return;

    double now = nps_utils_get_time_sec();
    ...
    if (s->bytes_this_sec >= s->limit_rate) {
        ...
        if (elapsed < 1.0) {
            nps_sleep_ms((unsigned long)((1.0 - elapsed) * 1000));
        }
        s->last_throttle_time = nps_utils_get_time_sec();
        s->bytes_this_sec = 0;
    }
}

static int fill_buffer(NpsStream *s) {
    if (s->read_buf.pos < s->read_buf.len) {
        return (int)(s->read_buf.len - s->read_buf.pos);
    }

    size_t to_read = NPS_STREAM_BUFFER_SIZE;
    if (s->limit_rate > 0) {
    ...
    int n;
    if (s->tls) {
        n = nps_tls_read(s->tls, s->read_buf.data, (int)to_read);
    } else {
    ...
    if (n > 0) {
        ...
        if (err == WSAETIMEDOUT) return -NPS_ERR_TIMEOUT;
#else
        if (errno == ETIMEDOUT || errno == EAGAIN || errno == EWOULDBLOCK) return -NPS_ERR_TIMEOUT;
#endif
        return -NPS_ERR_NETWORK;
    } else {
        s->read_buf.pos = 0;
        s->read_buf.len = 0;
    }
...
int nps_stream_read_exact(NpsStream *s, void *buf, size_t len) {
    ...
    while (total_read < len) {
        int n = nps_stream_read(s, p + total_read, len - total_read);
        if (n <= 0) return n;
        total_read += n;
    }
...
int nps_stream_write(NpsStream *s, const void *buf, size_t len) {
    ...
    int n;
    if (s->tls) {
        n = nps_tls_write(s->tls, buf, (int)to_write);
    } else {
