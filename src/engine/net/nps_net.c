...
int nps_net_connect_ex(const char *hostname, int port, unsigned int connect_timeout_sec, nps_err_t *out_err) {
    ...
    if (getaddrinfo(hostname, port_str, &hints, &result) != 0) {
        if (out_err) *out_err = NPS_ERR_RESOLVE;
        return -1;
    }
    ...
    freeaddrinfo(result);
    if (socket_fd == -1) {
        if (out_err) *out_err = NPS_ERR_CONNECT;
        return -1;
    }
...
int nps_net_connect_proxy_ex(
            ...
            if (th) {
                *th = '\0';
                ...
                if (tp) {
                    *tp = '\0';
                    int p_val = atoi(p + 1);
                    if (nps_strcasecmp(h, host) == 0 && p_val == port) {
    ...
    if (use_proxy && no_proxy) {
        ...
        if (np_copy) {
            char *trimmed_np = nps_utils_trim(np_copy);
            char *tok = trimmed_np;
            while (tok) {
                char *comma = strchr(tok, ',');
                if (comma) *comma = '\0';
                char *item = nps_utils_trim(tok);
                if (item) {
                    size_t hlen = strlen(host);
                    size_t ilen = strlen(item);
                    if (nps_strcasecmp(host, item) == 0) {
                        use_proxy = false;
                        break;
                    }
                    if (item[0] == '.' && hlen > ilen && nps_strcasecmp(host + hlen - ilen, item) == 0) {
                        use_proxy = false;
                        break;
                    }
                }
    ...
    if (!use_proxy) {
        int res = nps_net_connect_ex(target_host, target_port, connect_timeout_sec, out_err);
    ...
    int proxy_port = 0;
    if (nps_utils_parse_url(proxy, &proxy_scheme, &proxy_host, &proxy_port, &proxy_path) != 0) {
        if (out_err) *out_err = NPS_ERR_URL;
        if (allocated_target_host) free(allocated_target_host);
        return -1;
    }
    free(proxy_scheme);
    ...
    int socket_fd = nps_net_connect_ex(proxy_host, proxy_port, connect_timeout_sec, out_err);
    free(proxy_host);
    ...
    NpsBuf connect_req;
    nps_buf_init(&connect_req);
    nps_buf_printf(&connect_req, "CONNECT %s:%d HTTP/1.1\r\nHost: %s:%d\r\n", target_host, target_port, target_host, target_port);
    if (allocated_target_host) free(allocated_target_host);

    if (proxy_user) {
        char *auth_b64 = nps_utils_base64_encode((const unsigned char *)proxy_user, strlen(proxy_user));
        if (auth_b64) {
            nps_buf_printf(&connect_req, "Proxy-Authorization: Basic %s\r\n", auth_b64);
            free(auth_b64);
        }
    }

    nps_buf_append(&connect_req, "\r\n", 2);

    char *req_data = connect_req.data;
    size_t req_len = connect_req.len;

    if (socket_write(socket_fd, req_data, req_len) <= 0) {
        nps_buf_free(&connect_req);
        if (out_err) *out_err = NPS_ERR_NETWORK;
        socket_close(socket_fd);
        return -1;
    }
    nps_buf_free(&connect_req);
    ...
    if (strstr(resp_buf, " 200") == NULL) {
        if (out_err) *out_err = NPS_ERR_PROXY;
        socket_close(socket_fd);
        return -1;
    }
