...
NpsMultipart *nps_multipart_new(void) {
    NpsMultipart *m = calloc(1, sizeof(NpsMultipart));
    if (!m) return NULL;
...
void nps_multipart_add_file(NpsMultipart *m, const char *field_name, const char *filepath, const char *mime_type) {
...
const char *nps_multipart_content_type(const NpsMultipart *m) {
    if (((NpsMultipart *)m)->cached_content_type) free(((NpsMultipart *)m)->cached_content_type);
    char buf[128];
    snprintf(buf, sizeof(buf), "multipart/form-data; boundary=%s", m->boundary);
    ((NpsMultipart *)m)->cached_content_type = strdup(buf);
...
void nps_multipart_into_request(NpsMultipart *m, NpsRequest *req) {
    ...
    req->body_parts = calloc(max_parts, sizeof(NpsBodyPart));
    ...
    for (size_t i = 0; i < m->count; i++) {
        ...
        req->body_parts[req->body_parts_count].type = NPS_BODY_PART_MEM;
        ...
        if (p->is_file) {
            ...
            req->body_parts[req->body_parts_count].type = NPS_BODY_PART_FILE;
    ...
    nps_headermap_set(req->headers, "Content-Type", nps_multipart_content_type(m));
    return;

oom_cleanup:
    /* Free any already-allocated parts and signal failure */
    for (size_t j = 0; j < req->body_parts_count; j++) {
        if (req->body_parts[j].type == NPS_BODY_PART_MEM) {
            free((void *)req->body_parts[j].data);
        } else if (req->body_parts[j].type == NPS_BODY_PART_FILE) {
